...
public class DetailpageDaoSqls {
	...
	public static final String SELECT_PROMOTION_BY_ID = "SELECT description, fi.id AS fileId FROM file_info fi, product WHERE product.id = :productId and fi.id IN"
	...
	public static final String SELECT_USER_RIVIEW = "SELECT prod.description, RPAD(SUBSTR(ri.reservation_email, 1, 4), 8, '*') AS id, ruc.score, ruc.comment, DATE_FORMAT(ruc.modify_date, '%Y-%m-%d') AS modify_date, IFNULL(fi.id,0) AS fileId"
