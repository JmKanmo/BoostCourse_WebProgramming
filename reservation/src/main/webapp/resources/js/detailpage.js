...
const promotionObj = {
  ...
  updateSequence: function (sign) {
    ...
    if (sign === 1) {
      if (n + 1 > this.slideLen) n = 1;
      else n++;
    } else {
      if (n - 1 === 0) n = this.slideLen;
      else n--;
    }
  ...
  moveImageEnd: function (idx, sign) {
    if (sign === -1) {
      this.imgObject[idx]["pos"] =
        (this.slideLen - this.imgObject[idx]["idx"]) * this.imgList.offsetWidth;
    } else {
      this.imgObject[idx]["pos"] =
        -this.imgObject[idx]["idx"] * this.imgList.offsetWidth;
    }
  ...
  slideImageRight: function (param) {
    ...
    for (let i = 0; i < this.slideLen; i++) {
      if (
        ...
        -this.imgObject[i]["idx"] * this.imgList.offsetWidth
      ) {
        leftOut = i;
      }
    }
    ...
    for (let i = this.imgObject.length - 1, flag = false; i >= 0; i--) {
      ...
      if (
        this.imgObject[next]["pos"] <=
        -this.imgObject[next]["idx"] * this.imgList.offsetWidth
      ) {
  ...
  slideImageLeft: function (param) {
    ...
    for (let i = 0; i < this.slideLen; i++) {
      ...
      if (
        this.imgObject[i]["pos"] ===
        -(this.imgObject[i]["idx"] - 1) * this.imgList.offsetWidth
      ) {
        leftEnd = i;
      }
    }
...
const reservationObj = {
  ...
  updateCommentGrade: function (JSONData) {
    document.querySelector(".graph_value").style.width = `${
      JSONData["avgAndCnt"]["scoreAvg"] * 20
    }%`;
    document.querySelector(".user_comment_grade").innerText =
      JSONData["avgAndCnt"]["scoreAvg"];
    document.querySelector(
      ".join_count .green"
    ).innerText = `${JSONData["avgAndCnt"]["reviewCnt"]}건`;
  },

  updateUserComment: function (JSONData) {
    let slicedData = JSONData["review"].reduce(
      function (init_val, cur_val, idx) {
        if (idx < 3) {
          if (cur_val["fileURL"] === "") {
            cur_val["blind"] = "blind";
          } else {
